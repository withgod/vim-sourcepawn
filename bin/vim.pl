#!/usr/bin/perl -w
#
# ./bin/vim.pl ~/games/sourcemod-1.4.1-mac/addons/sourcemod/scripting/include
#
use strict;
...
if (\$ARGV != 1 && !-d $ARGV[0]) {
	die "missing parameter. require script parameter. \$SOURCEMOD/scripting/include";
}

my @files = <$ARGV[0]/*.inc>;

#these header and footer strings come from naris' original SourcePawn syntax file
#skip way down below them for actual code
my $current_date = strftime("%Y-%m-%d %H:%M:%S %z", localtime());
my $sm_version  = `grep SOURCEMOD_VERSION $ARGV[0]/version.inc | awk '{print \$3}' | sed 's/"//g'`;
my $header = <<"HEADER";
" Vim syntax file
" Language:	SourcePawn
